...
const target = `        const activeProductForOrder = products.find(p => p.id === confirmModal.order.productId);
...
const replacement = `        const activeProductForOrder = products.find(p => p.id === confirmModal.order.productId);
