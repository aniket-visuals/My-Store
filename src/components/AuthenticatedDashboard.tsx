    ...
    <div className="min-h-screen w-full bg-[#f8fafc] text-slate-800 flex flex-col md:flex-row font-sans relative overflow-x-hidden selection:bg-blue-100 selection:text-blue-900">
      ...
      <div className="md:hidden w-full bg-white border-b border-slate-100 px-5 py-4 flex items-center justify-between sticky top-0 z-30">
        ...
        <div className="flex items-center space-x-3">
          <img
            src={getUserAvatarUrl(selectedAvatar)}
            alt="Avatar"
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-2xl border border-slate-100 p-6 flex flex-col sm:flex-row items-center gap-6 shadow-sm">
                  <div className="relative shrink-0">
                    <div
                      className={`w-28 h-28 rounded-full ${getAvatarBgColor(
                        selectedAvatar
                      )} p-1.5 shadow-md flex items-center justify-center overflow-hidden border-2 border-white`}
                    >
                      <img
                        src={getUserAvatarUrl(selectedAvatar)}
                        alt="Ronald Richards"
                    ...
                    {/* Interactive Edit Indicator Badge */}
                    <button
                      onClick={() => setIsAvatarSelectorOpen(!isAvatarSelectorOpen)}
                      className="absolute bottom-1 right-1 bg-white hover:bg-slate-100 p-2 rounded-full shadow-md border border-slate-200 text-slate-700 transition-all cursor-pointer"
                      title="Change Avatar"
                    >
                  ...
                  <div className="text-center sm:text-left flex-1 space-y-3">
                    <div className="space-y-1">
                      <h3 className="font-bold text-sm text-slate-800">Upload new photo</h3>
                      <p className="text-xs text-slate-400">At least 800×800 px recommended. JPG or PNG is allowed</p>
                    </div>

                    <div className="flex flex-wrap gap-2.5 justify-center sm:justify-start">
                      <button
                        onClick={() => setIsAvatarSelectorOpen(!isAvatarSelectorOpen)}
                        className="px-4 py-2 bg-slate-50 hover:bg-slate-100 text-slate-700 font-bold text-xs rounded-xl border border-slate-200/80 transition-colors cursor-pointer"
                      >
                        {isAvatarSelectorOpen ? "Close selection" : "Select from list"}
                      </button>
                    </div>

                    {/* AVATAR CHOOSER SELECTOR GRID */}
                    <AnimatePresence>
                      {isAvatarSelectorOpen && (
                        <motion.div
                          ...
                          className="bg-slate-50 border border-slate-200/60 rounded-xl p-4 mt-3 text-left"
                        >
                          <p className="text-[10px] uppercase font-black tracking-wider text-slate-400 mb-3">
                            Choose profile picture style:
                          </p>
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            {[
                              { key: "ronald", label: "Ronald (Portrait)", bg: "bg-amber-400" },
                              { key: "cat", label: "Cute Cat", bg: "bg-pink-100" },
                              { key: "designer", label: "UX Designer", bg: "bg-indigo-100" },
                              { key: "pixel", label: "Developer", bg: "bg-slate-100" },
                            ].map((item) => (
                              <button
                                key={item.key}
                                onClick={() => handleSelectAvatar(item.key)}
                                className={`p-2.5 rounded-xl border flex flex-col items-center space-y-2 transition-all cursor-pointer ${
                              ...
                              >
                                <div
                                  className={`w-11 h-11 rounded-full ${item.bg} p-0.5 overflow-hidden flex items-center justify-center`}
                                >
                                  <img
                                ...
                                <span className="text-[9px] font-black text-slate-600 truncate max-w-full">
                                  {item.label}
                                </span>
                              </button>
                            ))}
                ...
                {/* CARD 2: PROFILE DETAILS (CONSOLIDATED PERSONAL INFO, LOCATION & BIO) */}
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="relative">
                            ...
                            <input
                              ...
                              onChange={(e) => setTempProfileLocation(e.target.value)}
                              className="w-full text-xs font-semibold text-slate-800 bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 focus:bg-white focus:border-blue-500 focus:ring-1 focus:ring-blue-100 outline-none transition-all"
                              placeholder="Location name (e.g. California)"
                            />
                          </div>
                        </div>
                      </div>
